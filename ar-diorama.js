...
(function(){
  ...
  function build(){if(built)return;built=true;
    ...
    let px=0,py=0;addEventListener('deviceorientation',e=>{px=(e.gamma||0)*.018;py=(e.beta||0)*.008});addEventListener('pointermove',e=>{px=(e.clientX/innerWidth-.5)*.28;py=(e.clientY/innerHeight-.5)*.12});
    ...
    const clock=new T.Clock();function tick(){const t=clock.getElapsedTime();root.rotation.y+=(px-root.rotation.y)*.035;root.rotation.x=-.08+(py-root.rotation.x+.08)*.025;boat.position.y=-.55+Math.sin(t*2)*.06;boat.rotation.z=Math.sin(t*2)*.04;gem.rotation.y=t*.9;lamp.material.emissive && lamp.material.emissive.setHex(0xffcc55);renderer.render(scene,camera);requestAnimationFrame(tick)}tick();
  }wait();
})();
